...
//Implementing slider banner & nav dots
    if (homeIsNotRunning) {
        ...
        // If already initialized, just reset the slider and interval
        clearInterval(interval);
        $("#button1").prop("checked", true);
        $(".summary1").css({visibility: "visible"});
        $("#content").css({"min-height": $(".sidebar").height() - 10});
        // Optionally, can call startSlider() here if want to restart the slider
    }
};
